...
class ASManager {
 public:
  ...
  void createASForScene(VulkanDevice* device, VkCommandPool commandPool,
                        Scene* scene);

  void createBLAS(VulkanDevice* device, VkCommandPool commandPool,
                  Scene* scene);

  void createTLAS(VulkanDevice* device, VkCommandPool commandPool,
                  Scene* scene);
 ...
 private:
  ...
  void createSingleBlas(VulkanDevice* device, VkCommandBuffer command_buffer,
                        BuildAccelerationStructure& build_as_structure,
                        VkDeviceAddress scratch_device_or_host_address);

  void createAccelerationStructureInfosBLAS(
