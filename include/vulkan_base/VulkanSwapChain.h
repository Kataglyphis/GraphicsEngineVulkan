...
class VulkanSwapChain {
 public:
  ...
  void initVulkanContext(VulkanDevice* device, Window* window,
                         const VkSurfaceKHR& surface);

  const VkSwapchainKHR& getSwapChain() const { return swapchain; };
  uint32_t getNumberSwapChainImages() const {
    return static_cast<uint32_t>(swap_chain_images.size());
  };
 ...
 private:
  ...
  VkSurfaceFormatKHR choose_best_surface_format(
      const std::vector<VkSurfaceFormatKHR>& formats);
  VkPresentModeKHR choose_best_presentation_mode(
