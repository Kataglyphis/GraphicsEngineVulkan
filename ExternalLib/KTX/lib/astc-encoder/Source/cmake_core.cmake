...
target_include_directories(${ASTC_TARGET}-static PUBLIC $<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}>
                                                        $<INSTALL_INTERFACE:.>)
...
if(${CLI})
  ...
  target_link_libraries(${ASTC_TARGET} PRIVATE ${ASTC_TARGET}-static)
endif()
...
macro(astcenc_set_properties NAME)
  ...
  target_compile_definitions(${NAME} PRIVATE # MSVC defines
                                             $<$<CXX_COMPILER_ID:MSVC>:_CRT_SECURE_NO_WARNINGS>)

  if(${DECOMPRESSOR})
    target_compile_definitions(${NAME} PRIVATE ASTCENC_DECOMPRESS_ONLY)
  endif()

  if(${DIAGNOSTICS})
    target_compile_definitions(${NAME} PUBLIC ASTCENC_DIAGNOSTICS)
  endif()

  target_compile_options(
    ${NAME}
    PRIVATE # Use pthreads on Linux/macOS
  ...
  if(${ENABLE_ASAN})
    target_compile_options(${NAME} PRIVATE $<$<CXX_COMPILER_ID:${CLANG_LIKE}>:-fsanitize=address>)

    target_link_options(${NAME} PRIVATE $<$<CXX_COMPILER_ID:${CLANG_LIKE}>:-fsanitize=address>)
  endif()
  ...
  elseif((${ISA_SIMD} MATCHES "sse2") OR (${UNIVERSAL_BUILD} AND ${ISA_SSE2}))
    ...
    target_compile_options(
      ${NAME} PRIVATE $<$<CXX_COMPILER_ID:AppleClang>:-msse2> $<$<CXX_COMPILER_ID:AppleClang>:-mno-sse4.1>
  ...
  elseif((${ISA_SIMD} MATCHES "sse4.1") OR (${UNIVERSAL_BUILD} AND ${ISA_SSE41}))
    ...
    # Suppress unused argument for macOS universal build behavior
    target_compile_options(${NAME} PRIVATE $<$<NOT:$<CXX_COMPILER_ID:MSVC>>:-msse4.1 -mpopcnt>
                                           $<$<CXX_COMPILER_ID:AppleClang>:-Wno-unused-command-line-argument>)
  ...
  elseif((${ISA_SIMD} MATCHES "avx2") OR (${UNIVERSAL_BUILD} AND ${ISA_AVX2}))
    ...
    target_compile_options(
      ${NAME}
      PRIVATE $<$<NOT:$<CXX_COMPILER_ID:MSVC>>:-mavx2
              -mpopcnt
              -mf16c>
...
if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
  ...
  set_source_files_properties(astcenccli_image_external.cpp PROPERTIES COMPILE_FLAGS ${EXTERNAL_CXX_FLAGS})
endif()
...
if(${CLI})
  ...
  target_include_directories(${ASTC_TARGET} PRIVATE ${CMAKE_CURRENT_BINARY_DIR})
