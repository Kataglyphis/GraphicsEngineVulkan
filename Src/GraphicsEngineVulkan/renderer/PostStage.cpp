...
void PostStage::recordCommands(
  ...
  render_pass_begin_info.renderArea.extent =
      swap_chain_extent;  // size of region to run render pass on (starting at
                          // offset)
  ...
  auto aspectRatio = static_cast<float>(swap_chain_extent.width) /
  ...
  vkCmdPushConstants(commandBuffer, pipeline_layout,
                     ...
                     0, sizeof(PushConstantPost), &pc_post);
...
void PostStage::cleanUp() {
  ...
  vkDestroySampler(device->getLogicalDevice(), offscreenTextureSampler,
                   nullptr);
...
void PostStage::createDepthbufferImage() {
  ...
  depth_format = choose_supported_format(
  ...
  // MIP LEVELS: for depth texture we only want 1 level :)
  depthBufferImage.createImageView(device, depth_format,
...
void PostStage::createOffscreenTextureSampler() {
  ...
  VkResult result =
      vkCreateSampler(device->getLogicalDevice(), &sampler_create_info, nullptr,
...
void PostStage::createGraphicsPipeline(
  ...
  File vertexShaderFile(
      shaderHelper.getShaderSpvDir(post_shader_dir.str(), post_vert_shader));
  std::vector<char> vertex_shader_code = vertexShaderFile.readCharSequence();
  File fragmentShaderFile(
      shaderHelper.getShaderSpvDir(post_shader_dir.str(), post_frag_shader));
  std::vector<char> fragment_shader_code =
      fragmentShaderFile.readCharSequence();
  ...
  color_blending_create_info.logicOpEnable =
...
void PostStage::createFramebuffer() {
  ...
  for (size_t i = 0; i < vulkanSwapChain->getNumberSwapChainImages(); i++) {
    ...
    std::array<VkImageView, 2> attachments = {swap_chain_image.getImageView(),
                                              depthBufferImage.getImageView()};

    VkFramebufferCreateInfo frame_buffer_create_info{};
    frame_buffer_create_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    frame_buffer_create_info.renderPass =
        render_pass;  // render pass layout the framebuffer will be used with
    frame_buffer_create_info.attachmentCount =
        static_cast<uint32_t>(attachments.size());
    frame_buffer_create_info.pAttachments =
        attachments.data();  // list of attachments (1:1 with render pass)
    const VkExtent2D& swap_chain_extent = vulkanSwapChain->getSwapChainExtent();
    frame_buffer_create_info.width =
        swap_chain_extent.width;  // framebuffer width
    frame_buffer_create_info.height =
        swap_chain_extent.height;         // framebuffer height
