...
void ASManager::createASForScene(VulkanDevice* device,
                                 VkCommandPool commandPool, Scene* scene) {
  this->vulkanDevice = device;
  createBLAS(device, commandPool, scene);
  createTLAS(device, commandPool, scene);
}

void ASManager::createBLAS(VulkanDevice* device, VkCommandPool commandPool,
                           Scene* scene) {
  ...
  PFN_vkGetBufferDeviceAddressKHR pvkGetBufferDeviceAddressKHR =
      (PFN_vkGetBufferDeviceAddressKHR)vkGetDeviceProcAddr(
          device->getLogicalDevice(), "vkGetBufferDeviceAddress");

  std::vector<BlasInput> blas_input(scene->getModelCount());

  for (uint32_t model_index = 0;
       model_index < static_cast<uint32_t>(scene->getModelCount());
       model_index++) {
    ...
    blas_input[model_index].as_build_offset_info.reserve(
        mesh_model->getMeshCount());

    for (size_t mesh_index = 0; mesh_index < mesh_model->getMeshCount();
         mesh_index++) {
      VkAccelerationStructureGeometryKHR acceleration_structure_geometry{};
      VkAccelerationStructureBuildRangeInfoKHR
          acceleration_structure_build_range_info{};

      objectToVkGeometryKHR(device, mesh_model->getMesh(mesh_index),
      ...
      blas_input[model_index].as_geometry.push_back(
          acceleration_structure_geometry);
      blas_input[model_index].as_build_offset_info.push_back(
          acceleration_structure_build_range_info);
    }
  }
  ...
  for (unsigned int i = 0; i < scene->getModelCount(); i++) {
    ...
    createAccelerationStructureInfosBLAS(device, build_as_structures[i],
                                         blas_input[i], current_scretch_size,
                                         current_size);
  ...
  scratchBuffer.create(device, max_scratch_size,
  ...
  VkDeviceAddress scratch_buffer_address = pvkGetBufferDeviceAddressKHR(
      device->getLogicalDevice(), &scratch_buffer_device_address_info);
  ...
  for (size_t i = 0; i < scene->getModelCount(); i++) {
    createSingleBlas(device, command_buffer, build_as_structures[i],
  ...
  commandBufferManager.endAndSubmitCommandBuffer(
      device->getLogicalDevice(), commandPool, device->getGraphicsQueue(),
      command_buffer);
...
void ASManager::createTLAS(VulkanDevice* device, VkCommandPool commandPool,
                           Scene* scene) {
  // LOAD ALL NECESSARY FUNCTIONS STRAIGHT IN THE BEGINNING
  // all functionality from extensions has to be loaded in the beginning
  // we need a reference to the device location of our geometry laying on the
  ...
  for (size_t model_index = 0; model_index < scene->getModelCount();
    ...
    // here
    glm::mat4 transpose_transform =
        glm::transpose(scene->getModelMatrix(static_cast<int>(model_index)));
    ...
    geometry_instance.instanceCustomIndex =
        model_index;  // gl_InstanceCustomIndexEXT
    ...
    geometry_instance.instanceShaderBindingTableRecordOffset =
  ...
  vulkanBufferManager.createBufferAndUploadVectorOnDevice(
      device, commandPool, geometryInstanceBuffer,
      VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
  ...
  geometry_instance_buffer_device_address_info.buffer =
      geometryInstanceBuffer.getBuffer();
  ...
  acceleration_structure_geometry_instances_data.data.deviceAddress =
      geometry_instance_buffer_address;

  VkAccelerationStructureGeometryKHR topAS_acceleration_structure_geometry{};
  topAS_acceleration_structure_geometry.sType =
      VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR;
  topAS_acceleration_structure_geometry.pNext = nullptr;
  topAS_acceleration_structure_geometry.geometryType =
      VK_GEOMETRY_TYPE_INSTANCES_KHR;
  topAS_acceleration_structure_geometry.geometry.instances =
      acceleration_structure_geometry_instances_data;
  ...
  acceleration_structure_build_geometry_info.mode =
      VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR;
  acceleration_structure_build_geometry_info.srcAccelerationStructure =
      VK_NULL_HANDLE;
  acceleration_structure_build_geometry_info.geometryCount = 1;
  acceleration_structure_build_geometry_info.pGeometries =
      &topAS_acceleration_structure_geometry;
  ...
  pvkGetAccelerationStructureBuildSizesKHR(
      ...
      &acceleration_structure_build_geometry_info, &count_instance,
      &acceleration_structure_build_sizes_info);

  // now we got the sizes
  VulkanBuffer& tlasVulkanBuffer = tlas.vulkanBuffer;
  tlasVulkanBuffer.create(
  ...
  // update build info
  acceleration_structure_build_geometry_info.scratchData.deviceAddress =
      scratch_buffer_address;
  acceleration_structure_build_geometry_info.srcAccelerationStructure =
      VK_NULL_HANDLE;
  ...
  VkAccelerationStructureBuildRangeInfoKHR
      acceleration_structure_build_range_info{};
  acceleration_structure_build_range_info.primitiveCount =
      scene->getModelCount();
  ...
  pvkCmdBuildAccelerationStructuresKHR(
      command_buffer, 1, &acceleration_structure_build_geometry_info,
      &acceleration_structure_build_range_infos);
...
void ASManager::cleanUp() {
  ...
  pvkDestroyAccelerationStructureKHR(vulkanDevice->getLogicalDevice(),
...
void ASManager::createSingleBlas(
    VulkanDevice* device, VkCommandBuffer command_buffer,
  ...
  blasVulkanBuffer.create(
      device, build_as_structure.size_info.accelerationStructureSize,
      VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR |
          VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
          VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT |
...
void ASManager::createAccelerationStructureInfosBLAS(
  ...
  build_as_structure.build_info.sType =
      VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_GEOMETRY_INFO_KHR;
  build_as_structure.build_info.type =
      VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR;
  build_as_structure.build_info.flags =
      VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR;
  build_as_structure.build_info.mode =
      VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR;
  build_as_structure.build_info.geometryCount =
      static_cast<uint32_t>(blas_input.as_geometry.size());
  ...
  build_as_structure.size_info.sType =
      VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR;

  std::vector<uint32_t> max_primitive_cnt(
      blas_input.as_build_offset_info.size());

  for (uint32_t temp = 0;
       temp < static_cast<uint32_t>(blas_input.as_build_offset_info.size());
       temp++)
    max_primitive_cnt[temp] =
        blas_input.as_build_offset_info[temp].primitiveCount;

  pvkGetAccelerationStructureBuildSizesKHR(
      device->getLogicalDevice(),
      VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
      &build_as_structure.build_info, max_primitive_cnt.data(),
...
void ASManager::objectToVkGeometryKHR(
  ...
  vertex_buffer_device_address_info.sType =
      VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
  ...
  index_buffer_device_address_info.sType =
      VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
  ...
  VkDeviceAddress vertex_buffer_address = pvkGetBufferDeviceAddressKHR(
      device->getLogicalDevice(), &vertex_buffer_device_address_info);
  VkDeviceAddress index_buffer_address = pvkGetBufferDeviceAddressKHR(
      device->getLogicalDevice(), &index_buffer_device_address_info);
  ...
  acceleration_structure_triangles_data.sType =
      VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_TRIANGLES_DATA_KHR;
  acceleration_structure_triangles_data.pNext = nullptr;
  acceleration_structure_triangles_data.vertexFormat =
      VK_FORMAT_R32G32B32_SFLOAT;
  acceleration_structure_triangles_data.vertexData =
  ...
  acceleration_structure_triangles_data.indexData =
      index_device_or_host_address_const;
  ...
  acceleration_structure_geometry_data.triangles =
      acceleration_structure_triangles_data;
