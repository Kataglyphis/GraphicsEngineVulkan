...
void ShaderHelper::compileShader(const std::string& shader_src_dir,
  ...
  log_stdout_and_stderr << " > " << shader_log_file.str() << " 2> "
                        << shader_log_file.str();
...
VkShaderModule ShaderHelper::createShaderModule(VulkanDevice* device,
  ...
  shader_module_create_info.pCode =
      reinterpret_cast<const uint32_t*>(code.data());  // pointer to code

  VkShaderModule shader_module;
  VkResult result =
      vkCreateShaderModule(device->getLogicalDevice(),
