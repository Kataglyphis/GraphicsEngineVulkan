...
std::array<VkVertexInputAttributeDescription, 4> getVertexInputAttributeDesc() {
  ...
  attribute_describtions[0].format =
      VK_FORMAT_R32G32B32_SFLOAT;  // format data will take (also helps define
                                   // size of data)
  ...
  attribute_describtions[1].format =
      VK_FORMAT_R32G32B32_SFLOAT;  // format data will take (also helps define
                                   // size of data)
  attribute_describtions[1].offset =
      offsetof(Vertex, normal);  // where this attribute is defined in the data
                                 // for a single vertex

  // normal coord attribute
  attribute_describtions[2].binding = 0;
  attribute_describtions[2].location = 2;
  attribute_describtions[2].format =
      VK_FORMAT_R32G32B32_SFLOAT;  // format data will take (also helps define
                                   // size of data)
  ...
  attribute_describtions[3].format =
      VK_FORMAT_R32G32_SFLOAT;  // format data will take (also helps define size
                                // of data)
  attribute_describtions[3].offset =
      offsetof(Vertex, texture_coords);  // where this attribute is defined in
