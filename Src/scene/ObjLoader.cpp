...
std::shared_ptr<Model> ObjLoader::loadModel(const std::string& modelFile) {
  ...
  new_model->add_new_mesh(device, transfer_queue, command_pool, vertices,
                          indices, materialIndex, this->materials);
...
void ObjLoader::loadVertices(const std::string& fileName) {
  ...
  if (attrib.normals.empty()) {
    for (size_t i = 0; i < indices.size(); i += 3) {
      ...
      glm::vec3 n =
          glm::normalize(glm::cross((v1.pos - v0.pos), (v2.pos - v0.pos)));
      v0.normal = n;
      v1.normal = n;
      v2.normal = n;
    }
