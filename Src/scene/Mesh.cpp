...
Mesh::Mesh(VulkanDevice* device, VkQueue transfer_queue,
  ...
  object_description.index_address =
      vkGetBufferDeviceAddress(device->getLogicalDevice(), &index_info);
  object_description.vertex_address =
      vkGetBufferDeviceAddress(device->getLogicalDevice(), &vertex_info);
  object_description.material_index_address = vkGetBufferDeviceAddress(
      device->getLogicalDevice(), &material_index_info);
  object_description.material_address =
      vkGetBufferDeviceAddress(device->getLogicalDevice(), &material_info);
...
void Mesh::createVertexBuffer(VkQueue transfer_queue,
  ...
  vulkanBufferManager.createBufferAndUploadVectorOnDevice(
      device, transfer_command_pool, vertexBuffer,
...
void Mesh::createIndexBuffer(VkQueue transfer_queue,
                             ...
                             std::vector<uint32_t>& indices) {
  vulkanBufferManager.createBufferAndUploadVectorOnDevice(
      device, transfer_command_pool, indexBuffer,
...
void Mesh::createMaterialIDBuffer(VkQueue transfer_queue,
                                  ...
                                  std::vector<unsigned int>& materialIndex) {
  vulkanBufferManager.createBufferAndUploadVectorOnDevice(
      device, transfer_command_pool, materialIdsBuffer,
      VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT |
          VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
          VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR |
          VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
...
void Mesh::createMaterialBuffer(VkQueue transfer_queue,
                                ...
                                std::vector<ObjMaterial>& materials) {
  vulkanBufferManager.createBufferAndUploadVectorOnDevice(
      device, transfer_command_pool, materialsBuffer,
      VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT |
          VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
          VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR |
          VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
