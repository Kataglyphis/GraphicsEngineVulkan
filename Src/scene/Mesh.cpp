	...
	std::vector<unsigned int>& materialIndex, std::vector<ObjMaterial>&	materials)
{
	...
	material_info.buffer = materials_buffer;
	...
	object_description.material_address			= vkGetBufferDeviceAddress(logical_device, &material_info);
...
void Mesh::create_material_buffer(VkQueue transfer_queue, VkCommandPool transfer_command_pool, std::vector<ObjMaterial>* materials)
{
	...
	void* data;																																			// 1.) create pointer to a point in normal memory
	vkMapMemory(device, staging_buffer_memory, 0, buffer_size, 0, &data);							// 2.) map the vertex buffer memory to that point
	memcpy(data, materials->data(), (size_t)buffer_size);																	// 3.) copy memory from vertices vector to the point
	vkUnmapMemory(device, staging_buffer_memory);																	// 4.) unmap the vertex buffer memory
	...
	create_buffer(physical_device, device, buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT |
		...
		VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT,
		&materials_buffer,
		&materials_buffer_memory);

	// copy staging buffer to vertex buffer on GPU
	copy_buffer(device, transfer_queue, transfer_command_pool, staging_buffer, materials_buffer, buffer_size);
