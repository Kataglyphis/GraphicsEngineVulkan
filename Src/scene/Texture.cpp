...
void Texture::createFromFile(VulkanDevice* device, VkCommandPool commandPool,
  ...
  void* data;
  vkMapMemory(device->getLogicalDevice(), stagingBuffer.getBufferMemory(), 0,
              size, 0, &data);
  ...
  createImage(device, width, height, mip_levels, VK_FORMAT_R8G8B8A8_UNORM,
  ...
  // copy data to image
  vulkanBufferManager.copyImageBuffer(
      device->getLogicalDevice(), device->getGraphicsQueue(), commandPool,
      stagingBuffer.getBuffer(), vulkanImage.getImage(), width, height);

  // generate mipmaps
  generateMipMaps(device->getPhysicalDevice(), device->getLogicalDevice(),
                  commandPool, device->getGraphicsQueue(),
                  vulkanImage.getImage(), VK_FORMAT_R8G8B8A8_SRGB, width,
                  height, mip_levels);

  stagingBuffer.cleanUp();

  createImageView(device, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT,
                  mip_levels);
}
...
void Texture::createImage(VulkanDevice* device, uint32_t width, uint32_t height,
                          uint32_t mip_levels, VkFormat format,
                          VkImageTiling tiling, VkImageUsageFlags use_flags,
                          VkMemoryPropertyFlags prop_flags) {
  vulkanImage.create(device, width, height, mip_levels, format, tiling,
                     use_flags, prop_flags);
}

void Texture::createImageView(VulkanDevice* device, VkFormat format,
                              VkImageAspectFlags aspect_flags,
                              uint32_t mip_levels) {
  vulkanImageView.create(device, vulkanImage.getImage(), format, aspect_flags,
                         mip_levels);
}
...
stbi_uc* Texture::loadTextureData(const std::string& file_name, int* width,
  ...
  stbi_uc* image =
      stbi_load(file_name.c_str(), width, height, &channels, STBI_rgb_alpha);
...
void Texture::generateMipMaps(VkPhysicalDevice physical_device, VkDevice device,
  ...
  VkCommandBuffer command_buffer =
      commandBufferManager.beginCommandBuffer(device, command_pool);
  ...
  for (uint32_t i = 1; i < mip_levels; i++) {
    ...
    blit.dstOffsets[1] = {tmp_width > 1 ? tmp_width / 2 : 1,
                          tmp_height > 1 ? tmp_height / 2 : 1, 1};
  ...
  commandBufferManager.endAndSubmitCommandBuffer(device, command_pool, queue,
                                                 command_buffer);
}
