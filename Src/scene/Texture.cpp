...
void Texture::createFromFile(VulkanDevice* device, VkCommandPool commandPool, const std::string& fileName)
{
  ...
  vulkanBufferManager.copyImageBuffer(
    device->getLogicalDevice(), device->getGraphicsQueue(), commandPool, stagingBuffer.getBuffer(), vulkanImage.getImage(), width, height);
...
void Texture::createImage(VulkanDevice* device, uint32_t width, uint32_t height, uint32_t mip_levels, VkFormat format, VkImageTiling tiling,
...
void Texture::createImageView(VulkanDevice* device, VkFormat format, VkImageAspectFlags aspect_flags, uint32_t mip_levels)
{
...
void Texture::generateMipMaps(VkPhysicalDevice physical_device, VkDevice device, VkCommandPool command_pool, VkQueue queue, VkImage image,
