...
void VulkanSwapChain::initVulkanContext(VulkanDevice* device, Window* window, const VkSurfaceKHR& surface)
{
  ...
  if (swap_chain_details.surface_capabilities.maxImageCount > 0 && swap_chain_details.surface_capabilities.maxImageCount < image_count) {
  ...
  swap_chain_create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT |
