...
void VulkanSwapChain::initVulkanContext(VulkanDevice* device, Window* window,
  ...
  VkSurfaceFormatKHR surface_format =
      choose_best_surface_format(swap_chain_details.formats);
  VkPresentModeKHR present_mode =
      choose_best_presentation_mode(swap_chain_details.presentation_mode);
  VkExtent2D extent =
      choose_swap_extent(swap_chain_details.surface_capabilities);

  // how many images are in the swap chain; get 1 more than the minimum to allow
  // tiple buffering
  uint32_t image_count =
      swap_chain_details.surface_capabilities.minImageCount + 1;
  ...
  swap_chain_create_info.imageFormat =
      surface_format.format;  // swapchain format
  swap_chain_create_info.imageColorSpace =
      surface_format.colorSpace;  // swapchain color space
  swap_chain_create_info.presentMode =
      present_mode;                             // swapchain presentation mode
  swap_chain_create_info.imageExtent = extent;  // swapchain image extents
  swap_chain_create_info.minImageCount =
      image_count;  // minimum images in swapchain
  ...
  swap_chain_create_info.preTransform =
      swap_chain_details.surface_capabilities
          .currentTransform;  // transform to perform on swap chain images
  swap_chain_create_info.compositeAlpha =
      VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;  // dont do blending; everything opaque
  ...
  if (indices.graphics_family != indices.presentation_family) {
    uint32_t queue_family_indices[] = {(uint32_t)indices.graphics_family,
    ...
    swap_chain_create_info.pQueueFamilyIndices =
        queue_family_indices;  // array of queues to share between
  ...
  VkResult result = vkCreateSwapchainKHR(
      device->getLogicalDevice(), &swap_chain_create_info, nullptr, &swapchain);
  ...
  for (size_t i = 0; i < images.size(); i++) {
    ...
    swap_chain_image.createImageView(device, swap_chain_image_format,
...
void VulkanSwapChain::cleanUp() {
  for (Texture& image : swap_chain_images) {
    vkDestroyImageView(device->getLogicalDevice(), image.getImageView(),
                       nullptr);
  }
