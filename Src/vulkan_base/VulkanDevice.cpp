...
void VulkanDevice::get_physical_device() {
  ...
  vkEnumeratePhysicalDevices(instance->getVulkanInstance(), &device_count,
                             nullptr);

  // if no devices available, then none support of Vulkan
  if (device_count == 0) {
    throw std::runtime_error(
        "Can not find GPU's that support Vulkan Instance!");
  }
...
void VulkanDevice::create_logical_device() {
  ...
  std::set<int> queue_family_indices = {indices.graphics_family,
  ...
  for (int queue_family_index : queue_family_indices) {
    ...
    queue_create_info.queueFamilyIndex =
        queue_family_index;  // the index of the family to create a queue from
  ...
  extensions.insert(extensions.begin(),
                    device_extensions_for_raytracing.begin(),
  ...
  device_create_info.queueCreateInfoCount = static_cast<uint32_t>(
      queue_create_infos.size());  // number of queue create infos
  device_create_info.pQueueCreateInfos =
      queue_create_infos.data();  // list of queue create infos so device can
                                  // create required queues
  device_create_info.enabledExtensionCount = static_cast<uint32_t>(
      extensions.size());  // number of enabled logical device extensions
  device_create_info.ppEnabledExtensionNames =
      extensions.data();  // list of enabled logical device extensions
  ...
  vkGetDeviceQueue(logical_device, indices.presentation_family, 0,
                   &presentation_queue);
...
SwapChainDetails VulkanDevice::getSwapchainDetails(VkPhysicalDevice device) {
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      device, *surface, &swapchain_details.surface_capabilities);
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(device, *surface,
                                            &presentation_count, nullptr);

  // if presentation modes returned, get list of presentation modes
  if (presentation_count > 0) {
    swapchain_details.presentation_mode.resize(presentation_count);
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        device, *surface, &presentation_count,
        swapchain_details.presentation_mode.data());
  }
...
bool VulkanDevice::check_device_suitable(VkPhysicalDevice device) {
  ...
  if (extensions_supported) {
    ...
    swap_chain_valid = !swap_chain_details.presentation_mode.empty() &&
