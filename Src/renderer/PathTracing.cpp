...
void PathTracing::init(
  ...
  computeLimits.maxComputeWorkGroupCount[0] =
      physicalDeviceProps.limits.maxComputeWorkGroupCount[0];
  computeLimits.maxComputeWorkGroupCount[1] =
      physicalDeviceProps.limits.maxComputeWorkGroupCount[1];
  computeLimits.maxComputeWorkGroupCount[2] =
      physicalDeviceProps.limits.maxComputeWorkGroupCount[2];

  computeLimits.maxComputeWorkGroupInvocations =
      physicalDeviceProps.limits.maxComputeWorkGroupInvocations;

  computeLimits.maxComputeWorkGroupSize[0] =
      physicalDeviceProps.limits.maxComputeWorkGroupSize[0];
  computeLimits.maxComputeWorkGroupSize[1] =
      physicalDeviceProps.limits.maxComputeWorkGroupSize[1];
  computeLimits.maxComputeWorkGroupSize[2] =
      physicalDeviceProps.limits.maxComputeWorkGroupSize[2];
...
void PathTracing::recordCommands(
    VkCommandBuffer& commandBuffer, uint32_t image_index,
    VulkanImage& vulkanImage, VulkanSwapChain* vulkanSwapChain,
  ...
  vkCmdWriteTimestamp(
      commandBuffer,
      VkPipelineStageFlagBits::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, queryPool,
      query++);
  ...
  presentToPathTracingImageBarrier.sType =
      VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  presentToPathTracingImageBarrier.pNext = nullptr;
  presentToPathTracingImageBarrier.srcQueueFamilyIndex =
  ...
  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT,
  ...
  vkCmdPushConstants(commandBuffer, pipeline_layout,
                     ...
                     sizeof(PushConstantPathTracing), &push_constant);
  ...
  uint32_t workGroupCountX =
      std::max((imageSize.width + specializationData.specWorkGroupSizeX - 1) /
                   specializationData.specWorkGroupSizeX,
               1U);
  uint32_t workGroupCountY =
      std::max((imageSize.height + specializationData.specWorkGroupSizeY - 1) /
                   specializationData.specWorkGroupSizeY,
               1U);
  uint32_t workGroupCountZ = 1;

  vkCmdDispatch(commandBuffer, workGroupCountX, workGroupCountY,
                workGroupCountZ);

  VkImageMemoryBarrier pathTracingToPresentImageBarrier{};
  pathTracingToPresentImageBarrier.sType =
      VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  ...
  pathTracingToPresentImageBarrier.dstQueueFamilyIndex =
      indices.graphics_family;
  ...
  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                       ...
                       nullptr, 1, &pathTracingToPresentImageBarrier);
  ...
  VkResult result = vkGetQueryPoolResults(
      device->getLogicalDevice(), queryPool, 0, query_count,
      queryResults.size() * sizeof(uint64_t), queryResults.data(),
      static_cast<VkDeviceSize>(sizeof(uint64_t)), VK_QUERY_RESULT_64_BIT);

  if (result != VK_NOT_READY) {
    pathTracingTiming = (static_cast<float>(queryResults[1] - queryResults[0]) *
                         timeStampPeriod) /
                        1000000.f;
  }
}
...
void PathTracing::createQueryPool() {
  ...
  ASSERT_VULKAN(vkCreateQueryPool(device->getLogicalDevice(), &queryPoolInfo,
                                  NULL, &queryPool),
...
void PathTracing::createPipeline(
  ...
  compute_pipeline_layout_create_info.sType =
      VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
  compute_pipeline_layout_create_info.setLayoutCount =
      static_cast<uint32_t>(descriptorSetLayouts.size());
  compute_pipeline_layout_create_info.pushConstantRangeCount = 1;
  compute_pipeline_layout_create_info.pPushConstantRanges =
      &push_constant_range;
  compute_pipeline_layout_create_info.pSetLayouts = descriptorSetLayouts.data();

  ASSERT_VULKAN(vkCreatePipelineLayout(device->getLogicalDevice(),
  ...
  File pathTracingShaderFile(shaderHelper.getShaderSpvDir(
      pathTracing_shader_dir.str(), pathTracing_shader));
  std::vector<char> pathTracingShadercode =
      pathTracingShaderFile.readCharSequence();

  shaderHelper.compileShader(pathTracing_shader_dir.str(), pathTracing_shader);

  // build shader modules to link to graphics pipeline
  VkShaderModule pathTracingModule =
      shaderHelper.createShaderModule(device, pathTracingShadercode);
  ...
  compute_shader_integrate_create_info.sType =
  ...
  ASSERT_VULKAN(vkCreateComputePipelines(
                    ...
                    &compute_pipeline_create_info, nullptr, &pipeline),
