...
void PathTracing::recordCommands(VkCommandBuffer& commandBuffer, uint32_t image_index, VulkanImage& vulkanImage, VulkanSwapChain* vulkanSwapChain,
  const std::vector<VkDescriptorSet>& descriptorSets)
{
  ...
  vkCmdWriteTimestamp(commandBuffer, VkPipelineStageFlagBits::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, queryPool, query++);
  ...
  vkCmdPushConstants(commandBuffer, pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(PushConstantPathTracing), &push_constant);

  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);

  vkCmdBindDescriptorSets(
    commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout, 0, static_cast<uint32_t>(descriptorSets.size()), descriptorSets.data(), 0, 0);
  ...
  vkCmdPipelineBarrier(
    commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &pathTracingToPresentImageBarrier);
...
void PathTracing::createQueryPool()
{
  ...
  ASSERT_VULKAN(vkCreateQueryPool(device->getLogicalDevice(), &queryPoolInfo, NULL, &queryPool), "Failed to create query pool!");
}
...
void PathTracing::createPipeline(const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts)
{
  ...
  ASSERT_VULKAN(vkCreatePipelineLayout(device->getLogicalDevice(), &compute_pipeline_layout_create_info, nullptr, &pipeline_layout),
  ...
  ASSERT_VULKAN(vkCreateComputePipelines(device->getLogicalDevice(), VK_NULL_HANDLE, 1, &compute_pipeline_create_info, nullptr, &pipeline),
