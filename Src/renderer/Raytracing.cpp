...
void Raytracing::recordCommands(VkCommandBuffer& commandBuffer, VulkanSwapChain* vulkanSwapChain, const std::vector<VkDescriptorSet>& descriptorSets)
{
  ...
  PFN_vkGetBufferDeviceAddressKHR vkGetBufferDeviceAddressKHR =
    reinterpret_cast<PFN_vkGetBufferDeviceAddressKHR>(vkGetDeviceProcAddr(device->getLogicalDevice(), "vkGetBufferDeviceAddressKHR"));

  PFN_vkCmdTraceRaysKHR pvkCmdTraceRaysKHR = (PFN_vkCmdTraceRaysKHR)vkGetDeviceProcAddr(device->getLogicalDevice(), "vkCmdTraceRaysKHR");
  ...
  vkCmdBindDescriptorSets(
    commandBuffer, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, pipeline_layout, 0, static_cast<uint32_t>(descriptorSets.size()), descriptorSets.data(), 0, nullptr);

  const VkExtent2D& swap_chain_extent = vulkanSwapChain->getSwapChainExtent();
  pvkCmdTraceRaysKHR(commandBuffer, &rgen_region, &miss_region, &hit_region, &call_region, swap_chain_extent.width, swap_chain_extent.height, 1);
}
...
void Raytracing::createPCRange()
{
  ...
  pc_ranges.stageFlags = VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR;
  pc_ranges.offset = 0;
...
void Raytracing::createGraphicsPipeline(const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts)
{
  ...
  result = pvkCreateRayTracingPipelinesKHR(
    device->getLogicalDevice(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &raytracing_pipeline_create_info, nullptr, &graphicsPipeline);
...
void Raytracing::createSBT()
{
  ...
  VkResult result = pvkGetRayTracingShaderGroupHandlesKHR(device->getLogicalDevice(), graphicsPipeline, 0, group_count, sbt_size, handles.data());
