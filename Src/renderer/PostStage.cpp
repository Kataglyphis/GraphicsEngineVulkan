...
void PostStage::init(VulkanDevice* device, VulkanSwapChain* vulkanSwapChain, const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts)
{
  this->device = device;
...
void PostStage::recordCommands(VkCommandBuffer& commandBuffer, uint32_t image_index, const std::vector<VkDescriptorSet>& descriptorSets)
{
  ...
  // used framebuffer depends on the swap chain and therefore is changing for each command buffer
  ...
  vkCmdPushConstants(commandBuffer, pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstantPost), &pc_post);
