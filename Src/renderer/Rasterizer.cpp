...
void Rasterizer::init(
    VulkanDevice* device, VulkanSwapChain* vulkanSwapChain,
...
void Rasterizer::setPushConstant(PushConstantRasterizer pushConstant) {
  this->pushConstant = pushConstant;
}

void Rasterizer::recordCommands(
    VkCommandBuffer& commandBuffer, uint32_t image_index, Scene* scene,
  ...
  for (uint32_t m = 0; m < static_cast<uint32_t>(scene->getModelCount()); m++) {
    ...
    for (unsigned int k = 0; k < scene->getMeshCount(m); k++) {
      ...
      VkBuffer vertex_buffers[] = {
          scene->getVertexBuffer(m, k)};  // buffers to bind
      VkDeviceSize offsets[] = {0};
      vkCmdBindVertexBuffers(
          commandBuffer, 0, 1, vertex_buffers,
          offsets);  // command to bind vertex buffer before drawing with them

      // bind mesh index buffer with 0 offset and using the uint32 type
      vkCmdBindIndexBuffer(commandBuffer, scene->getIndexBuffer(m, k), 0,
      ...
      vkCmdDrawIndexed(commandBuffer,
                       static_cast<uint32_t>(scene->getIndexCount(m, k)), 1, 0,
                       0, 0);
    }
  }
...
void Rasterizer::createRenderPass() {
  ...
  color_attachment.format =
      swap_chain_image_format;  // format to use for attachment
  ...
  color_attachment.initialLayout =
      VK_IMAGE_LAYOUT_GENERAL;  // image data layout before render pass starts
  color_attachment.finalLayout =
      VK_IMAGE_LAYOUT_GENERAL;  // image data layout after render pass (to
                                // change to)

  // depth attachment of render pass
  VkAttachmentDescription depth_attachment{};
  depth_attachment.format = choose_supported_format(
  ...
  subpass_dependencies[0].srcSubpass =
      VK_SUBPASS_EXTERNAL;  // subpass index (VK_SUBPASS_EXTERNAL = Special
  ...
  subpass_dependencies[0].srcAccessMask =
      0;  // stage access mask (memory access)
  ...
  VkResult result =
      vkCreateRenderPass(device->getLogicalDevice(), &render_pass_create_info,
                         nullptr, &render_pass);
...
void Rasterizer::createFramebuffer() {
  ...
  for (size_t i = 0; i < framebuffer.size(); i++) {
    std::array<VkImageView, 2> attachments = {
        offscreenTextures[i].getImageView(), depthBufferImage.getImageView()};
    ...
    VkResult result = vkCreateFramebuffer(device->getLogicalDevice(),
...
void Rasterizer::createTextures(VkCommandPool& commandPool) {
  ...
  VkCommandBuffer cmdBuffer = commandBufferManager.beginCommandBuffer(
      device->getLogicalDevice(), commandPool);

  for (uint32_t index = 0;
       index <
       static_cast<uint32_t>(vulkanSwapChain->getNumberSwapChainImages());
       index++) {
    Texture texture{};
    const VkExtent2D& swap_chain_extent = vulkanSwapChain->getSwapChainExtent();
    const VkFormat& swap_chain_image_format =
    ...
    texture.createImage(
        device, swap_chain_extent.width, swap_chain_extent.height, 1,
        swap_chain_image_format, VK_IMAGE_TILING_OPTIMAL,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
            VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    texture.createImageView(device, swap_chain_image_format,
                            VK_IMAGE_ASPECT_COLOR_BIT, 1);

    // --- WE NEED A DIFFERENT LAYOUT FOR USAGE
    VulkanImage& image = texture.getVulkanImage();
    image.transitionImageLayout(cmdBuffer, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  VkFormat depth_format = choose_supported_format(
      device->getPhysicalDevice(),
      {VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D32_SFLOAT,
       VK_FORMAT_D24_UNORM_S8_UINT},
      VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
  ...
  const VkExtent2D& swap_chain_extent = vulkanSwapChain->getSwapChainExtent();
  depthBufferImage.createImage(device, swap_chain_extent.width,
                               swap_chain_extent.height, 1, depth_format,
  ...
  // MIP LEVELS: for depth texture we only want 1 level :)
  depthBufferImage.createImageView(
      device, depth_format,
  ...
  vulkanImage.transitionImageLayout(
      device->getLogicalDevice(), device->getGraphicsQueue(), commandPool,
...
void Rasterizer::createGraphicsPipeline(
  ...
  File vertexFile(
      shaderHelper.getShaderSpvDir(rasterizer_shader_dir.str(), "shader.vert"));
  File fragmentFile(
      shaderHelper.getShaderSpvDir(rasterizer_shader_dir.str(), "shader.frag"));
  ...
  VkShaderModule vertex_shader_module =
      shaderHelper.createShaderModule(device, vertex_shader_code);
  VkShaderModule fragment_shader_module =
      shaderHelper.createShaderModule(device, fragment_shader_code);

  // shader stage creation information
  // vertex stage creation information
  VkPipelineShaderStageCreateInfo vertex_shader_create_info{};
  vertex_shader_create_info.sType =
  ...
  fragment_shader_create_info.sType =
      VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  ...
  std::array<VkVertexInputAttributeDescription, 4> attribute_describtions =
      vertex::getVertexInputAttributeDesc();
  ...
  vertex_input_create_info.vertexAttributeDescriptionCount =
      static_cast<uint32_t>(attribute_describtions.size());
  vertex_input_create_info.pVertexAttributeDescriptions =
      attribute_describtions.data();
  ...
  result = vkCreateGraphicsPipelines(device->getLogicalDevice(), VK_NULL_HANDLE,
                                     1, &graphics_pipeline_create_info, nullptr,
                                     &graphics_pipeline);
  ASSERT_VULKAN(result, "Failed to create a graphics pipeline!")

  // Destroy shader modules, no longer needed after pipeline created
  vkDestroyShaderModule(device->getLogicalDevice(), vertex_shader_module,
                        nullptr);
  vkDestroyShaderModule(device->getLogicalDevice(), fragment_shader_module,
                        nullptr);
}
