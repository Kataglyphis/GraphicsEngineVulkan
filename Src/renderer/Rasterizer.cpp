...
void Rasterizer::init(
  VulkanDevice* device, VulkanSwapChain* vulkanSwapChain, const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts, VkCommandPool& commandPool)
{
...
void Rasterizer::recordCommands(VkCommandBuffer& commandBuffer, uint32_t image_index, Scene* scene, const std::vector<VkDescriptorSet>& descriptorSets)
{
  ...
  for (uint32_t m = 0; m < static_cast<uint32_t>(scene->getModelCount()); m++) {
    ...
    for (unsigned int k = 0; k < scene->getMeshCount(m); k++) {
      ...
      vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertex_buffers, offsets); // command to bind vertex buffer before drawing with them
      ...
      vkCmdBindDescriptorSets(
        commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, static_cast<uint32_t>(descriptorSets.size()), descriptorSets.data(), 0, nullptr);
...
void Rasterizer::createFramebuffer()
{
  ...
  for (size_t i = 0; i < framebuffer.size(); i++) {
    ...
    std::array<VkImageView, 2> attachments = { offscreenTextures[i].getImageView(), depthBufferImage.getImageView() };
    ...
    VkResult result = vkCreateFramebuffer(device->getLogicalDevice(), &frame_buffer_create_info, nullptr, &framebuffer[i]);
...
void Rasterizer::createTextures(VkCommandPool& commandPool)
{
  ...
  for (uint32_t index = 0; index < static_cast<uint32_t>(vulkanSwapChain->getNumberSwapChainImages()); index++) {
    ...
    image.transitionImageLayout(cmdBuffer, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, 1, VK_IMAGE_ASPECT_COLOR_BIT);
...
void Rasterizer::createGraphicsPipeline(const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts)
{
  ...
  std::vector<VkPipelineShaderStageCreateInfo> shader_stages = { vertex_shader_create_info, fragment_shader_create_info };
